...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        if (name) {
            ...
            const template = `<medium class="font-garamond mt-0 mb-1 mx-0 p-0">${'Dear, '+util.escapeHtml(guestName?.getAttribute('data-message'))}</small><p class="font-garamond m-0 p-0" style="font-size: 2rem">${util.escapeHtml(name)}</p>`;
